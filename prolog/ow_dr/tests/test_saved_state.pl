...
test(restored_configuration_falls_back_only_when_sidecars_are_absent,
    ...
    Captured=Initial.settings.put(_{startupConfigured:true,startupFiles:['KBs/missing.krf']}),
    ...
    setup_call_cleanup(asserta(kb_saved_state:image_manifest(Meta),Ref),
      (kb_saved_state:effective_configuration(Restored),
       assertion(Restored.settings.startupFiles==['KBs/missing.krf']),
       assertion(Restored.settings.revision==none),
       assertion(Restored.sourcePacks.packs==Packs),
