...
:- dynamic teacher_timing_request/2.
...
test(request_ids_follow_rounds_without_rewriting_completed_steps) :-
    uuid(Run),kb_llm_timing:begin_run("timing-request-fixture",Run),
    kb_llm_timing:with_run(Run,
      (kb_llm_timing:request_id("request-one"),kb_llm_timing:phase(request_preparation),
       kb_llm_timing:instant(http_dispatch),kb_llm_timing:phase(endpoint_wait),
       kb_llm_timing:request_id("request-two"),kb_llm_timing:phase(request_preparation),
       kb_llm_timing:instant(http_dispatch),kb_llm_timing:phase(endpoint_wait))),
    kb_llm_timing:checkpoint(Run,"completed","reply",_),
    kb_llm_timing:finish(Run),
    kb_llm_timing:latest_terminal("timing-request-fixture",Trace),
    assertion(Trace.requestId=="request-two"),teacher_timing_trace(Trace),
    findall(Id,(member(S,Trace.steps),S.operation=="http_dispatch",Id=S.requestId),Ids),
    assertion(Ids==["request-one","request-two"]),
...
test(actual_endpoint_wait_and_terminal_trace_add_no_http_or_state_writes,
    ...
    setup_call_cleanup(
      ...
      (start_chat(_{id:C.id,revision:C.revision,text:"Synthetic timing-only sentinel",approvedNonsensitive:true},_),
       ...
       assertion(string(Active.requestId)),string_length(Active.requestId,36),
       assertion(Active.currentStep.requestId==Active.requestId),
       ...
       assertion(\+get_dict(requestId,Request,_)),assertion(Trace.requestId==Active.requestId),
...
test(actual_tool_steps_keep_receipts_local_and_add_no_provider_rounds,
    ...
    findall(B,fixture_request(chat,B),Before),length(Before,BeforeCount),
    retractall(user:teacher_timing_request(_,_)),
    setup_call_cleanup(
      wrap_predicate(kb_llm_transport:chat_completion(Config,_,_,_),teacher_timing_request_ids,Wrapped,
        (assertz(user:teacher_timing_request(Config.conversation,Config.requestId)),call(Wrapped))),
      bound_chat(C,"Synthetic timed managed task",Preview,Done),
      unwrap_predicate(kb_llm_transport:chat_completion(_,_,_,_),teacher_timing_request_ids)),
    ...
    findall(Id,teacher_timing_request(C.id,Id),RequestIds),
    RequestIds=[FirstId,SecondId],assertion(FirstId\==SecondId),
    assertion(T.requestId==SecondId),
    forall(member(Id,RequestIds),(assertion(string(Id)),string_length(Id,36))),
    findall(Id,(member(S,T.steps),S.operation=="http_dispatch",Id=S.requestId),TraceIds),
    assertion(TraceIds==RequestIds),
