:- module(kb_inventory_rules,[rule_classification/2,project_retained_forms/2,
                             rule_parts/2,conclusion_head/4,
                             logical_operator/1,quantifier/1,valid_binder/1]).
...
rule_classification(Term,Info) :-
    rule_parts(Term,Parts),
    (Parts\=[]->
      (Parts=[rule(Kind,Executable,_,_)]->true
      ;Kind=grouped_rules,(member(rule(_,true,_,_),Parts)->Executable=true;Executable=false)),
      findall(Head,(member(rule(_,_,_,Conclusions),Parts),
                    member(Role-Conclusion,Conclusions),conclusion_head(Conclusion,Role,positive,Head)),Heads),
...
% Return shared, untouched source terms. A wrapper is not itself a definition:
% only an enclosed rule contributes conclusions; negated rules remain data.
rule_parts(Term,Parts) :-
    (var(Term)->Parts=[]
    ;Term=(Head:-Body),nonvar(Body),Body=..[and|Goals]->
      Parts=[rule(executable,true,Goals,[head-Head])]
    ;compound(Term),Term=..[Name,Binder,Inner],quantifier(Name),valid_binder(Binder)->
      rule_parts(Inner,Parts)
    ;compound(Term),Term=..[Name|Children],memberchk(Name,[x_and,x_or])->
      maplist(rule_parts,Children,Groups),append(Groups,Parts)
    ;compound(Term),Term=..[Name|Args],formula_rule(Name,Args,Kind,Antecedents,Conclusions)->
      Parts=[rule(Kind,false,Antecedents,Conclusions)]
    ;Parts=[]).

formula_rule(Name,[Antecedent,Consequent],implication,[Antecedent],[consequent-Consequent]) :-
    memberchk(Name,[x_implies,'x_=>']),formula_value(Consequent).
formula_rule(Name,[Left,Right],biconditional,[Left,Right],[left-Left,right-Right]) :-
    memberchk(Name,[x_equiv,'x_<=>']),formula_value(Left),formula_value(Right).
formula_rule('x_<==',[Head|Body],two_equals_operator_data,Body,[head-Head]) :-
...
quantifier(Name) :- memberchk(Name,[x_forAll,x_forall,x_thereExists,x_exists]).
logical_operator(Name) :-
    (quantifier(Name);memberchk(Name,[x_and,x_or,x_not,x_implies,'x_=>',
                                    x_equiv,'x_<=>','x_<==','x_<==='])).
...
conclusion_head(Term,Role,Polarity,Head) :-
    var(Term),!,predicate_reference(Term,Reference),
    Head=Reference.put(_{arity:null,role:Role,polarity:Polarity}).
conclusion_head(Term,Role,Polarity,Head) :-
    atom(Term),formula_symbol(Term),\+logical_operator(Term),!,
...
conclusion_head(Term,Role,Polarity,Head) :-
    ...
    ;quantifier(Name),Args=[Binder,Child],valid_binder(Binder)->
      conclusion_head(Child,Role,Polarity,Head)
    ;memberchk(Name,[x_implies,'x_=>']),Args=[_,Child]->
      conclusion_head(Child,Role,Polarity,Head)
    ;memberchk(Name,[x_equiv,'x_<=>']),Args=[Left,Right]->
      member(Child,[Left,Right]),conclusion_head(Child,Role,Polarity,Head)
    ...
    ;formula_symbol(Name),\+logical_operator(Name),length(Args,Arity),
